...
namespace ParadoxNotion.Design
{
	...
	partial class EditorUtils
    {
        ...
        public static GenericMenu GetPreferedTypesSelectionMenu(Type baseType, Action<Type> callback, GenericMenu menu = null, string subCategory = null, bool showAddTypeOption = false) {
            ...
            var isGenericDefinition = baseType.IsGenericTypeDefinition && baseType.GetGenericArguments().Length == 1;
        ...
        public static MenuItemInfo[] GetMenuItems(GenericMenu menu) {
            ...
            var contentField = itemType.GetField("content");
            var separatorField = itemType.GetField("separator");
            var selectedField = itemType.GetField("on");
            var func1Field = itemType.GetField("func");
            var func2Field = itemType.GetField("func2");
            var dataField = itemType.GetField("userData");

            var result = new List<MenuItemInfo>();
            foreach ( var item in items ) {
                var content = contentField.GetValue(item) as GUIContent;
                var separator = (bool)separatorField.GetValue(item);
                var selected = (bool)selectedField.GetValue(item);
                var func1 = (GenericMenu.MenuFunction)func1Field.GetValue(item);
                var func2 = (GenericMenu.MenuFunction2)func2Field.GetValue(item);
                var userData = dataField.GetValue(item);
