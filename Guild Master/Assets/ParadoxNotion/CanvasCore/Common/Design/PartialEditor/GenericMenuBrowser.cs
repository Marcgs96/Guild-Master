...
namespace ParadoxNotion.Design
{
    ...
    public class GenericMenuBrowser : PopupWindowContent
    {
        ...
        void DoHeader(Rect headerRect, Event e) {
            //HEADER
            GUILayout.Space(5);
            GUILayout.Label(string.Format("<color=#{0}><size=14><b>{1}</b></size></color>", EditorGUIUtility.isProSkin ? "dddddd" : "222222", title), headerStyle);
        ...
        void DoTree(Rect treeRect, Event e) {
            ...
            if ( search != lastSearch ) {
                ...
                if ( !string.IsNullOrEmpty(search) ) {
                    ...
                    searchRootNode.children = leafNodes
                        ...
                        .OrderBy(x => StringUtils.ScoreSearchMatch(search, x.category, x.category) * ( x.isFavorite ? 0.5f : 1 ))
            ...
            foreach ( var childPair in currentNode.children ) {
                ...
                var text = string.Format("<color={0}><size=11>{1}</size></color>", hexColor, ( leafItem == null ? string.Format("<b>{0}</b>", label) : label ));
                GUILayout.Label(text, GUILayout.Width(0), GUILayout.ExpandWidth(true));
