...
namespace NodeCanvas.Editor
{
    ...
    public class TaskEditor : EditorObjectWrapper<Task>
    {
        ...
        void ShowAgentField() {
            ...
            var infoString = isMissingType && EditorGUIUtility.isProSkin ? "<color=#ff5f5f>" + task.agentType.FriendlyName() + "</color>" : task.agentType.FriendlyName();
            ...
            if ( task.agentIsOverride ) {
                ...
                if ( taskAgent.useBlackboard ) {
                    ...
                    if ( varNames.Contains(taskAgent.name) || string.IsNullOrEmpty(taskAgent.name) ) {
                        taskAgent.name = EditorUtils.StringPopup(GUIContent.none, taskAgent.name, varNames);
            ...
            if ( task.agentIsOverride ) {
                if ( isMissingType ) {
                    GUILayout.Label("(" + infoString + ")", GUILayout.Height(18));
                }
